...
class my_words extends StatefulWidget {
  final Controller_get cs = Get.put(Controller_get());

  @override
  _my_wordsState createState() => _my_wordsState();
}

//////13nisan!!!!!!---------------------------------------------hooooooooooooooooooppppppp------------------
///kelimeleri çektin liste boşsa error verdin
///şimdi kelimeleri çevir ve silip my words e eklemeyi getir.

class _my_wordsState extends State<my_words> {



  Future<File> write_words(String my_words_str) async {
    final file = await File(cs.all_word_path.toString());
    return file.writeAsString("$my_words_str", mode: FileMode.append);
  }
  ...
  void show_alert(){
    Alert(
      context: context,
      type: AlertType.info,
      style: AlertStyle(backgroundColor: colors().brown, titleStyle: TextStyle(color: colors().grey, fontSize: 25, fontFamily: "bahnschrift")),
      title: "You haven't any word",
      buttons: [
        DialogButton(
          child: Text(
            "OK",
            style: TextStyle(color: colors().brown, fontSize: 25, fontFamily: "bahnschrift"),
          ),
  ...
  Future<int> read_word() async {
    try {
      ...
      if (file.isBlank || file_2.length == 0) {
        ...
        var _list_str = await my_word_list[0].toString();
  ...
  void initState() {
    ...
    setState(() {
      read_word().whenComplete(() {
        make_translate_list(list: my_word_list);
      });
    });
  }
      ...
      home: ModalProgressHUD(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: little_text_grey(title: "My words", value: MediaQuery.of(context).size.width*15/411),
            actions: [
              Column(
                mainAxisAlignment: MainAxisAlignment.end,
                crossAxisAlignment: CrossAxisAlignment.center,
                children: [
                  FaIcon(FontAwesomeIcons.language,color: colors().grey,),
                  little_text_grey(title: "${cs.native_language.toString()} to ${cs.target_language.toString()}", value: MediaQuery.of(context).size.width*15/411),
                ],
              ),
              ...
              Column(
                ...
                children:<Widget>[
                  IconButton(icon: FaIcon(
                    ...
                    my_word_list = [];
                    show = true;
                    Get.to(()=>all_words()).whenComplete(() => Navigator.of(context).pop());
            ...
            children: [
              Expanded(
                child: ListView.separated(
                  ...
                  shrinkWrap: true,
                  itemCount: my_word_list.length == 0 ? 0 : my_word_list.length,
                    ...
                    return Slidable(
                      ...
                      actions: <Widget>[
                        IconSlideAction(
                          onTap: (){
                            setState(() {
                             write_words('"${my_word_list[index].toString()}",').whenComplete(() => delete_words(word: my_word_list[index]));
                              show_toast(message: "added to all words");
                      ...
                      secondaryActions: <Widget>[
                        IconSlideAction(
                          onTap: (){
                            setState(() {
                             delete_words(word: my_word_list[index]);
                             translated_words_list.remove(translated_words_list[index]);
